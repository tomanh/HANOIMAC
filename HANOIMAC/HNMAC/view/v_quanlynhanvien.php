...
<?php if($_SESSION['chucvu']=='Quản lý'&&!empty($_SESSION['ss_admin_nhanvien'])){ ?>
...
<div class="container">
	<div class="row col-sm-12">
		<div class="col-sm-4">
			...
			<input style="background: #4D5253;height: 40px; ; color: white;" name="btn-add" type="submit" value="+Thêm mới nhân viên" class="btn btn-md">
			</form>
			<?php if(isset($_POST['btn-add'])||!empty($error)){ ?>
			<form action="?controller=xulynhanvien&method=add" method="post" style="margin-top: 20px;">
			<div class="form-group">
				<label>Tên đầy đủ:</label><?php if (isset($error['fullname'])){ ?> <span style="color: red;"><?php echo $error['fullname'];?></span><?php } ?>
				<input class="form-control" type="text" name="fullname" value="<?php echo (isset($fullname))?$fullname:'' ?>">
			</div>
			<div class="form-group">
				<label>Tài khoản:</label><?php if (isset($error['username'])){ ?> <span style="color: red;"><?php echo $error['username'];?></span><?php } ?>
				<input class="form-control" type="text" name="username" value="<?php echo (isset($username))?$username:'' ?>">
			</div>
			<div class="form-group">
				<label>Mật khẩu:</label><?php if (isset($error['password'])){ ?> <span style="color: red;"><?php echo $error['password'];?></span><?php } ?>
				<input class="form-control" type="password" name="password" value="<?php echo (isset($password))?$password:'' ?>">
			</div>
			<div class="form-group">
				<input style="background: #4D5253;width: 35%; color: white;" type="submit" name="" value="+Thêm" class="btn btn-md">
			</div>
		...
		<div class="panel" style=" border: 1px solid #0c1a1e; width: 65%;float: right;">
			<div class="panel-heading" style="background: #4D5253; color: white; font-size: 16px;">Danh sách nhân viên cửa hàng HANOIMAC</div>
				...
				<table class="table table-hover">
					...
					<tbody>
						<tr>
							...
							<td style="width: 30%">
								<a style="background: #2E6EC8; color: white; width: 40%; height: 30px; line-height: 25px; " href="?controller=editnhanvien&id=<?php echo $value['id']?>&fullname=<?php echo $value['fullname']?>&username=<?php echo $value['username']?>&password=<?php echo $value['password']?>" class="btn btn-xs">Sửa</a>
								<a style="background: #F60E29; color: white; width: 40%; height: 30px; line-height: 25px; " href="?controller=xulynhanvien&method=delete&id=<?php echo $value['id']?>" class="btn btn-xs">Xoá</a>
							</td>						
						</tr>
					</tbody>
					<?php } ?>					
...
<?php }else { ?>
<h3>Đại vương của chúng tôi đang đi vắng! Bạn đang không có quyền truy cập!</h3>
<h3>Vui lòng quay lại! <a href="?controller=trangchu">Trang chủ</a></h3>
<?php } ?>
</body>
</html>